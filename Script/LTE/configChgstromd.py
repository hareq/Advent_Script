...
nodeBSysUpTimeoid=scriptinterface.getSnmpOidForNodeName(enodebip,161,'nodeBSysUpTime')
...
n = 0
while n < 1:
    nodeBSysDynamicCfgFileCreateTriggerNodeBOIDWithInstance=scriptinterface.getCurrentOIDWithInstance()
    nodeBSysDynamicCfgFileCreateTriggerNodeBOID=scriptinterface.getCurrentOID()
    nodeBSysDynamicCfgFileCreateTriggerNodeBInstance=nodeBSysDynamicCfgFileCreateTriggerNodeBOIDWithInstance.replace(nodeBSysDynamicCfgFileCreateTriggerNodeBOID,'')
    nodeBSysDynamicCfgFileCreateTriggerNodeB = scriptinterface.getNodeValue(nodeBSysDynamicCfgFileCreateTriggerNodeBOID,nodeBSysDynamicCfgFileCreateTriggerNodeBInstance)
    if nodeBSysDynamicCfgFileCreateTriggerNodeB == "1":
        values = array(['0',enodebid,"2011-11-11 11:11:11",nodeBSysUpTimeoid+'.0'+"|3|"+time.strftime('%Y-%m-%d %X',time.localtime(time.time()))],String)
        for m in range(0,1):
            scriptinterface.sendV2Trap(mgrName,162,"public",configChgTrapOID,objectid,nodetypes,values)
        time.sleep(1)
        n = 0
    if  nodeBSysDynamicCfgFileCreateTriggerNodeB =="0":
        print "off"
        n = 1

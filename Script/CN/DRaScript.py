...
def write_drafile():
        ...
        f = open("C:\AdventNet_config_file_cn\dra_staticDate.xml",'r+')
        dradate = f.read()
        dradate = dradate.replace('DraManagedElementIdvlue',from_filetrap_get_DraManagedElementId())
        dradate = dradate.replace('DraSctpAssocIdvlue',getAgentIp())
        dradate = dradate.replace('DraEthernetPortIpAddressListvlue',getAgentIp())
        
        f.close()
        f1 = open("DRA-NRM"+from_filetrap_get_DraManagedElementId()+".xml",'w+')
        f1.write(dradate)
        f1.close()
        
def write_draalarmfile():
        print 'Start write_draalarmfile()'
        f = open("C:\AdventNet_config_file_cn\dra_alarm.adb",'r+')
        dradate = f.read()
...
def from_filetrap_get_DraManagedElementId():
        ...
        if (fileTransferDestination.find('dra=') != 0):
           ...
           for n in range(0,fileTransferDestinationlen):
               if fileTransferDestination[n] == '/':
                    pcrfid = fileTransferDestination[4:n]
                    pcrfidoid = scriptinterface.getSnmpOidForNodeName(getAgentIp(),getAgentPort(),'DraManagedElementId')
                    scriptinterface.updateValue(pcrfidoid,'.0',pcrfid,"CONST")

        if (get_fileTransferFileName_vlue().find('dra=') != 0):
            fileTransferDestinationlen = len(get_fileTransferFileName_vlue())
            for n in range(0,fileTransferDestinationlen):
                if get_fileTransferFileName_vlue()[n] == '#':
                    pcrfid = get_fileTransferFileName_vlue()[4:n]
        else:
            pcrfidoid = scriptinterface.getSnmpOidForNodeName(getAgentIp(),getAgentPort(),'DraManagedElementId')
...
def from_alarmfiletrap_get_DraManagedElementId():
        ...
        for n in range(0,fileTransferDestinationlen):
                if fileTransferDestination[n] == '_':
                    draid = fileTransferDestination[4:n]
                    break
        return draid
...
def update_draid_into_adventnet():
...
def main():
        ...
        if (get_fileTransferType_vlue() == "1" and get_fileTransferFlag_vlue() =="1"):
                print 'Start dra_file_trap'
                e = 'true';
                try:
                        f = open("DRA-NRM"+from_filetrap_get_DraManagedElementId()+".xml",'r')
                        f.close()
                except :
                        e = 'false'
                        
                if e == 'true':
                        zipfile("DRA-NRM"+from_filetrap_get_DraManagedElementId()+".xml",get_fileTransferFileName_vlue())

                if e == 'false':
                        print 'Have no file,goto write_drafile()'
                        write_drafile()
                        zipfile('DRA-NRM'+from_filetrap_get_DraManagedElementId()+'.xml',get_fileTransferFileName_vlue())

...
def write_dynamicDate(enodebid):
    ...
    if antArrayVendorNameoid == None:
        ...
        f = open('C:\dynamicDate100.dcb','r+')
        dynamicDate100 = f.read()
        dynamicDate100 = dynamicDate100.replace('<sn>NodeBFunction=','<sn>NodeBFunction='+enodebidox)
        dynamicDate100 = dynamicDate100.replace('<nedn>NodeBFunction=','<nedn>NodeBFunction='+enodebidox)

        
        f.close()

        f1 = open(dcb,'w+')
        f1.write(dynamicDate100)
        f1.close()
        
    if antArrayVendorNameoid != None:
    
        f = open('C:\dynamicDate200.dcb','r+')
        dynamicDate200 = f.read()
        dynamicDate200 = dynamicDate200.replace('<sn>NodeBFunction=','<sn>NodeBFunction='+enodebidox)
        dynamicDate200 = dynamicDate200.replace('<nedn>NodeBFunction=','<nedn>NodeBFunction='+enodebidox)

        
        f.close()

        f1 = open(dcb,'w+')
        f1.write(dynamicDate200)
        f1.close()
...
def write_staticDate(enodebid):
    ...
    if antArrayVendorNameoid == None:
        ...
        f = open('C:\staticDate100.dcb','r+')
        staticDate100 = f.read()

        staticDate100 = staticDate100.replace(reenodebidiod,reenodebidiod+enodebid)
        staticDate100 = staticDate100.replace(renodeBGlobalIdoid,renodeBGlobalIdoid+enodebid)
        staticDate100 = staticDate100.replace(renodeBSysOMAddroid,renodeBSysOMAddroid+enodebip)
        staticDate100 = staticDate100.replace('<sn>NodeBFunction=','<sn>NodeBFunction='+enodebidox)
        staticDate100 = staticDate100.replace('<nedn>NodeBFunction=','<nedn>NodeBFunction='+enodebidox)

        
        f.close()

        f1 = open(dcb,'w+')
        f1.write(staticDate100)
        f1.close()
        
    if antArrayVendorNameoid != None:
    
        f = open('C:\staticDate200.dcb','r+')
        staticDate200 = f.read()

        staticDate200 = staticDate200.replace(reenodebidiod,reenodebidiod+enodebid)
        staticDate200 = staticDate200.replace(renodeBGlobalIdoid,renodeBGlobalIdoid+enodebid)
        staticDate200 = staticDate200.replace(renodeBSysOMAddroid,renodeBSysOMAddroid+enodebip)
        staticDate200 = staticDate200.replace('<sn>NodeBFunction=','<sn>NodeBFunction='+enodebidox)
        staticDate200 = staticDate200.replace('<nedn>NodeBFunction=','<nedn>NodeBFunction='+enodebidox)

        
        f.close()

        f1 = open(dcb,'w+')
        f1.write(staticDate200)
        f1.close()
